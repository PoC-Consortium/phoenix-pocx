...
**This section applies to Core-backed wallets** (managed / external node). Nodeless (remote / Electrum) and Android wallets handle older coins differently — there is no upgrade step at all — so see the note that follows.

On a Core-backed wallet you know it needs upgrading because the Wallets screen (and the toolbar wallet selector) show an amber **upgrade** button next to its name, whose tooltip reads *"To stay compatible with v31 wallets, this wallet needs to upgrade."* There is **no separate "v30" badge** in managed mode — the button is the only marker (the amber *v30* chip is a remote-mode thing, Chapter 26). The upgrade is always something *you* start — Phoenix never runs it automatically.

![A managed-mode wallet flagged for upgrade: an amber upgrade button sits next to the wallet name. There is no separate v30 badge in managed mode.](images/processed/ch05-core-upgrade-button.png){width=85%}

> **Note** — The upgrade button appears only for ordinary single-seed mainnet wallets. Multisig, watch-only, and single-key (WIF/descriptor) imports are left alone, and nothing is flagged on testnet or regtest.

The upgrade runs through an **Upgrade this wallet** dialog. It asks you to *re-enter this wallet's recovery phrase* to confirm the wallet is yours — *"your coins stay exactly where they are, and nothing is sent anywhere."* An amber note warns that the upgrade re-imports the wallet and runs a **blockchain rescan** that can take several minutes. Type the phrase (and your BIP39 passphrase, if the wallet has one); the **Upgrade** button enables once the phrase is valid. A phrase that does not match the wallet is rejected inline — *"That recovery phrase doesn't match this wallet."* On success Phoenix imports the v31 branch into the wallet, rescans it, and shows *"Wallet upgraded."*

![The Upgrade this wallet dialog: re-enter the recovery phrase to confirm ownership; an amber note warns of the rescan.](images/processed/ch05-upgrade-dialog.png){width=60%}

The upgrade happens **in place**: the same wallet now derives its addresses on the v31 path, its earlier coins stay exactly where they are, and the amber upgrade button disappears once it finishes. Nothing is sent on-chain and no second wallet is created.

> **Note — remote (Electrum) and Android wallets don't upgrade; they use pockets.** A nodeless wallet holds every branch of one recovery phrase together, as **pockets** of a single wallet (Chapter 26). Its current SegWit and Taproot pockets are already on the v31 standard, and older coins simply appear as a spend-only **v30** pocket alongside them — there is no *Upgrade to v31* action, and nothing to re-type. To bring old coins onto the standard path you open the v30 pocket and **Send** them to a current pocket. The **Check for older (v30) funds** action (Chapter 12) still applies: on these wallets a hit materialises the v30 pocket inside the wallet.
