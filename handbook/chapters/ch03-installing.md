...
This chapter walks you through downloading and installing Phoenix on your computer or phone. The process is short, but a few platform-specific quirks are worth knowing about — especially the security warnings that appear because the desktop installers are not yet code-signed.

> **Note** — You only need to install **Phoenix** itself. The Bitcoin-PoCX Core node, which Phoenix needs to talk to the network, is downloaded automatically on first launch (Chapter 4). You do not have to fetch it separately.
...
Always download from one of these two sources. Phoenix is *not* distributed through any third-party download site, browser extension store, app store (other than the Android channel described below), or social-media link.

> **Warning** — Phishing copies of cryptocurrency wallets are common. If you ever land on a "Phoenix" download page that does not match one of the URLs above, close the page. Bookmark the real download page in your browser so you can return to it directly.
...
```powershell
Get-FileHash .\Phoenix-Wallet_2.0.0_x64-setup.exe -Algorithm SHA256
```

**macOS / Linux:**

```bash
shasum -a 256 Phoenix-Wallet_2.0.0_x64.dmg     # macOS
sha256sum Phoenix-Wallet_2.0.0_amd64.AppImage  # Linux
```
...
Phoenix desktop installers are **not yet code-signed**. Because of this:
...
1. Download `Phoenix-Wallet_<version>_x64-setup.exe` from the website or GitHub Releases.
...
1. Download `Phoenix-Wallet_<version>_amd64.AppImage`.
2. (Recommended) Verify the SHA-256 checksum.
3. Make the file executable:

    ```bash
    chmod +x Phoenix-Wallet_2.0.0_amd64.AppImage
    ```

4. Run it:

    ```bash
    ./Phoenix-Wallet_2.0.0_amd64.AppImage
    ```
...
1. Download `Phoenix-Wallet_<version>_amd64.deb`.
2. (Recommended) Verify the SHA-256 checksum.
3. Install with `apt`:

    ```bash
    sudo apt install ./Phoenix-Wallet_2.0.0_amd64.deb
    ```
...
1. Download `Phoenix-Wallet_<version>_x86_64.rpm`.
2. (Recommended) Verify the SHA-256 checksum.
3. Install with `dnf` (Fedora/RHEL) or `zypper` (openSUSE):

    ```bash
    sudo dnf install ./Phoenix-Wallet_2.0.0_x86_64.rpm
    # or
    sudo zypper install ./Phoenix-Wallet_2.0.0_x86_64.rpm
    ```

## Installing on Android

The Android build is distributed as an APK on GitHub Releases — Phoenix is not currently published on Google Play or F-Droid.
...
5. Tap **Install**. After a moment, the wallet appears in your app drawer as **Phoenix Wallet**.
...
With Phoenix installed, you are ready to start it for the first time. The next chapter walks you through the first-launch wizard, where you choose between the **managed** and **external** node modes, pick a network, and watch the node sync to the rest of the chain.
