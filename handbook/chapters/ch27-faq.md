...
Bitcoin-PoCX has its own registered coin type (SLIP-44 `1347371864`); new mainnet wallets use it automatically (testnet/regtest use the standard test coin type `1'`), and restore still finds funds derived under the older Bitcoin coin type. It is invisible in normal use — there is nothing to set. The one thing you *might* see: a mainnet wallet made before this change is flagged as **v30** — offered a one-time upgrade on a Core wallet, or shown as a spend-only pocket in remote mode (next question). See Chapter 5.

**My wallet shows a "v30" badge — what does that mean, and should I upgrade?**
It means those coins derive at the old Bitcoin coin type rather than the registered BTCX one. They work fine as-is, so there is no rush — and what you actually see depends on your mode. On a **Core-backed** wallet (managed / external node) there is no badge; instead an **upgrade** button appears next to the wallet. Click it, re-enter the recovery phrase, and Phoenix imports the v31 branch into the same wallet and rescans — no coins move, the upgrade happens in place, and the button then clears (Chapter 5). On a **remote (Electrum) or Android** wallet there is no upgrade step at all: older coins appear as a spend-only **v30 pocket** of the wallet (tagged *"· v30"* in the pocket selector), alongside the current SegWit and Taproot pockets. To bring them onto the standard path, open the v30 pocket and **Send** them to a current pocket. See Chapters 5 and 26.

**What are the "pockets" under my remote / Android wallet?**
The different derivation branches of one recovery phrase, gathered under a single wallet. In remote (Electrum) and Android mode a wallet has a **SegWit** and a **Taproot** pocket (and, on mainnet, a spend-only **v30** pocket if older coins exist). Each pocket has its own balance and addresses; you pick the wallet with the **wallet selector** and the pocket with a separate **pocket selector** (a dropdown next to it). Only the SegWit pocket can mine or publish a forging assignment. See Chapter 26.

**I use one recovery phrase on two devices and a balance looked stuck — is that a bug?**
It was a known limitation that is now fixed: a wallet used on two machines will discover coins sent to addresses the *other* device handed out, and unconfirmed receives show up without waiting for the next block. If a balance still looks behind, check the Electrum status indicator (Chapter 26) — a stale or disconnected server is the usual cause.
