...
| **Miner**         | `hardware`  | Green while the miner is running and scanning plots, grey while stopped. Visible only after you have configured mining (Chapter 15). The same indicator appears in the Android wallet toolbar on hybrid installs.         |
...
| **Clock drift**   | `schedule`  | A small clock that colours green / amber / red according to how far your system clock has drifted from network time. Visible only when clock-drift monitoring is enabled (Chapter 12) **and** the wallet runs with a local node — drift matters for local forging, so remote (Electrum) mode does not show it. Click it to open the **System Clock Drift** dialog (Chapter 20). |
