...
@Component({
  ...
  template: `
    <div class="summary-cards">
      ...
      <div class="summary-card">
        <div class="card-header">
          <span class="card-title"><mat-icon>language</mat-icon>{{ 'aggregator_network_info' | i18n }}</span>
        </div>
        <div class="stat-row">
          <div class="stat-item">
      ...
      <div class="summary-card">
        <div class="card-header">
          <span class="card-title"><mat-icon>emoji_events</mat-icon>{{ 'aggregator_block_best' | i18n }}</span>
        </div>
        @if (hasBest()) {
          <div class="stat-row">
            <div class="stat-item">
      ...
      <div class="summary-card">
        <div class="card-header">
          <span class="card-title"><mat-icon>storage</mat-icon>{{ 'aggregator_capacity_estimate' | i18n }}</span>
        </div>
        <div class="capacity-value">{{ stats()?.totalCapacity ?? '0 B' }}</div>
        <div class="card-sub">
          {{ 'aggregator_stats_machines' | i18n: { machines: stats()?.activeMachines ?? 0 } }} · {{ 'aggregator_stats_accounts' | i18n: { accounts: stats()?.uniqueMiners ?? 0 } }}
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .btn-icon-glyph { font-size: 14px; line-height: 1; }
...
export class SummaryStatsComponent {
  ...
  getStatusText(): string {
    const s = this.status();
    switch (s.type) {
      case 'running': return this.i18n.get('aggregator_running');
      case 'starting': return this.i18n.get('aggregator_starting');
      case 'error': return this.i18n.get('aggregator_error');
      default: return this.i18n.get('aggregator_offline');
    }
  }
  ...
  getSubText(): string {
    const s = this.status();
    switch (s.type) {
      case 'running': return this.i18n.get('aggregator_listening_on', { address: s.listenAddress || '...' });
      case 'error': return s.message || this.i18n.get('aggregator_unknown_error');
      default: return this.i18n.get('aggregator_press_start');
    }
  }
}
