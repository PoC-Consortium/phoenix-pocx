...
import { MachineSummary } from '../../models/aggregator.models';
import { I18nPipe, I18nService } from '../../../core/i18n';

@Component({
  selector: 'app-aggregator-machines-section',
  standalone: true,
  imports: [MatIconModule, I18nPipe],
  template: `
    <div class="section">
      <div class="section-header">
        <span class="section-title">{{ 'aggregator_machines' | i18n }}</span>
      </div>
      <div class="section-content">
        @if (machines().length === 0) {
          <div class="no-data">{{ 'aggregator_no_machines' | i18n }}</div>
        } @else {
          <table>
            <thead>
              <tr>
                <th>{{ 'aggregator_machine_id' | i18n }}</th>
                <th>{{ 'aggregator_accounts' | i18n }}</th>
            ...
            <tbody>
              @for (row of machines(); track row.machineId) {
                <tr (click)="toggleExpand(row.machineId)">
                  <td class="mono">{{ truncate(row.machineId) }}</td>
          ...
          @if (expandedMachine(); as machine) {
            <div class="expanded-detail">
              <div class="detail-title">{{ getAccountsOnLabel(machine.machineId) }}</div>
              @for (acct of machine.accounts; track acct.accountId) {
...
export class MachinesSectionComponent {
  ...
  expandedMachine = () => {
    ...
    if (!id) return null;
    return this.machines().find(m => m.machineId === id) ?? null;
  };
  ...
  getAccountsOnLabel(machineId: string): string {
    return this.i18n.get('aggregator_accounts_on', { id: this.truncate(machineId) });
  }
}
